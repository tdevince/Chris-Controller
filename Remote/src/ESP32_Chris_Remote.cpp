...
uint32_t timeoutMillis=10000; //time to wait for new data once sent.
...
void normalOps()
{
  ...
  if (millis() - previousMillis >= interval) {
    ...
    if (FirstDraw)
      {
        ...
        if(O2Flow>1)
        {
          ...
          snprintf(buffer, sizeof(buffer), "%.1f", O2Flow); // Format O2Flow as a string
    ...
    if (O2Flow != O2FlowLast)   //flow value has changed
    {
      u8g2.setDrawColor(0);      
      memset(buffer, 0, sizeof(buffer));
      snprintf(buffer, sizeof(buffer), "%.1f", O2FlowLast); // Format O2Flow as a string
      u8g2.setFont(u8g2_font_helvB24_tr);
